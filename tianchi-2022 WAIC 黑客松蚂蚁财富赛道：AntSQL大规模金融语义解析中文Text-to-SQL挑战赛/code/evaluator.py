...
class HydraEvaluator():
    def __init__(self, output_path, config, hydra_featurizer, model):
        self.config = config
        self.model = model
    ...
    def _eval_imp(self, eval_data: SQLDataset, get_sq=True):
        items = ["overall", "sel", "wn", "wc", "op", "val"]
        ...
        for input_feature, model_output in zip(eval_data.input_features, model_outputs):
            ...
            select, where, conditions, where_num = self.model.parse_output(input_feature, model_output, wc_label)
            ...
            if ("DEBUG" in self.config or get_sq) and not all_correct:
                #try:
                    ...
                    pred_sq = input_feature.output_SQ(sel=select, conditions=[conditions[w] for w in where])
