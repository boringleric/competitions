...
with open('./banlist_v2.txt', 'r', encoding='utf-8') as f:
    bancontent = f.readlines()
    banlist = [c.strip() for c in bancontent]

with open('训练文件path，请替换', encoding='utf-8') as f:
    content = f.readlines()
...
schema_df = pd.DataFrame(pd.read_csv('./fundTable1.csv'))
...
with open('append_v5.jsonl','w',encoding='utf-8') as fw:    
